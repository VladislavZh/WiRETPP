...
class IntensityBank(nn.Module, ABC):
    ...
    def component_head_parameters(self) -> tuple[nn.Parameter, ...]:
        """Return parameters whose leading output blocks represent components."""
...
class StateIntensityBank(IntensityBank, ABC):
    ...
    def __init__(
        self, n_components: int, n_marks: int, integration_rule: IntegrationRule
    ) -> None:
        super().__init__()
    ...
    def rates_at_times(
        ...
        """Query intensities with optional observable temporal covariates."""
        return self.rates_from_states(states, elapsed)
    ...
    def _event_rows(
        ...
        time_tensor = torch.tensor(times, device=self.device, dtype=self.dtype)
        rates = self.rates_at_times(
            states[path_index, history_index],
            elapsed_tensor,
            time_tensor,
            path_index,
            sequences,
    ...
    def _integral_rows(
        ...
        return _IntegralRows(
            ...
            rates=self.rates_at_times(
                ...
                elapsed,
                start[:, None] + elapsed,
                path_index[:, None].expand(-1, sample_count),
                sequences,
            ).reshape(-1, self.n_components, self.n_marks),
        )
