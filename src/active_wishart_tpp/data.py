...
SYNTHETIC_DATASET_NAME = re.compile(
    "^(?:wishart_)?(?:(sin|trunc|markonly)(?:_sep)?_|longtail_h\\d{2,3}_)?K([2-5])_C5$"
)
...
class EventSequence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.regime_edges is not None:
            edges = self.regime_edges
            if (
                edges.ndim != 1
                or edges.size < 2
                ...
                or (not np.isclose(edges[-1], self.horizon))
            ):
                raise ValueError(
                    "regime edges must partition the full observation horizon"
                )
...
class P99TimeNormalizer:
    ...
    def fit(cls, train: tuple[EventSequence, ...]) -> TimeNormalization:
        ...
        if maximum_absolute >= 100000000.0:
            unit_rate = 1.0 / 86400.0
        ...
        return TimeNormalization(
            unit_rate, exponential_rate, f"{unit_mode}+train_p99_exponential"
        )
    ...
    def transform(
        ...
        origin = 0.0 if sequence.regime_edges is not None else float(sequence.times[0])
        ...
        edges = (
            None
            if sequence.regime_edges is None
            else sequence.regime_edges * normalization.rate
        )
        return EventSequence(times, sequence.marks.copy(), horizon, edges)
...
class EventDataModule:
    ...
    def _adapters():
        ...
        from active_wishart_tpp.data_adapters import (
            LabeledAdapter,
            OfficialSplitAdapter,
        )
