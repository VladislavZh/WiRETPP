...
import numpy as np
import pandas as pd
from active_wishart_tpp.data import (
    REAL_DATASETS,
...
class LabeledAdapter:
    ...
    def _metadata(name: str, labels: np.ndarray) -> tuple[int, int, float | None]:
        ...
        horizon = 20.0 if match.group(1) in {"sin", "trunc", "markonly"} else None
    ...
    def read(
        ...
        return DatasetSource(name, n_components, n_marks, sequences, labels, source_ids)
...
class OfficialSplitAdapter:
    ...
    def _parts(directory: Path, packed_filename: str) -> list[list[EventSequence]]:
        ...
        packed = _packed(
            directory, packed_filename, ("split", "source_id", "times", "marks")
        )
        if packed is None:
            return [
                [
        ...
        for split in ("train", "val", "test"):
            ...
            parts.append(
                [
                    _sequence(
                        _array(row.times, np.float64), _array(row.marks, np.int64)
                    )
