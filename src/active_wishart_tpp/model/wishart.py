...
def stable_cholesky(matrix: Tensor) -> Tensor:
    ...
    stabilized = (
        vectors * values.clamp_min(floor).unsqueeze(-2) @ vectors.transpose(-1, -2)
    )
...
def raw_cholesky_from_spd(matrix: Tensor, floor: float = 0.0001) -> Tensor:
    cholesky = torch.linalg.cholesky(0.5 * (matrix + matrix.transpose(-1, -2)))
    diagonal = inverse_softplus(
        (cholesky.diagonal(dim1=-2, dim2=-1) - floor).clamp_min(1e-08)
    )
    return torch.tril(cholesky, diagonal=-1) + torch.diag_embed(diagonal)


def spd_from_raw_cholesky(raw: Tensor, floor: float = 0.0001) -> Tensor:
...
def wishart_kl(
    posterior_mean: Tensor, posterior_df: Tensor, prior_mean: Tensor, prior_df: float
) -> Tensor:
...
def wishart_log_prob(
    draws: Tensor, mean: Tensor, degrees_of_freedom: Tensor | float
) -> Tensor:
...
def sample_wishart(
    mean: Tensor, degrees_of_freedom: Tensor, samples: int, generator: torch.Generator
) -> Tensor:
