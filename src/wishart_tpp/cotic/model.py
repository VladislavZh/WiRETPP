...
from wishart_tpp.backbones.integration import IntegrationRule, MonteCarloRule
from wishart_tpp.cotic.encoder import CoticEncoder
from wishart_tpp.cotic.head import CoticIntensityHead
        ...
        *,
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.intensity_head = CoticIntensityHead(
                ...
                base_mark_count=n_marks,
            )

    def expand_components(self, count: int, noise: float, seed: int) -> None:
        """Clone a fitted one-component head and perturb only its copies."""
        ...
        # Preserve the fitted first block; perturb only its new component copies.
        with torch.no_grad():
            weights = old_output.weight.repeat(count, 1)
            biases = old_output.bias.repeat(count)
            weight_noise = torch.randn(
                weights.shape, generator=generator, device=self.device
            )
            bias_noise = torch.randn(
                biases.shape, generator=generator, device=self.device
            )
            weight_noise[: self.n_marks].zero_()
            bias_noise[: self.n_marks].zero_()
            expanded_output.weight.copy_(weights + noise * weight_noise)
            expanded_output.bias.copy_(biases + noise * bias_noise)

        # Replace only the intensity head; the COTIC history encoder remains shared.
