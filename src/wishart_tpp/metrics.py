...
def clustering_summary(
    ...
    if np.asarray(labels).size and np.all(np.asarray(labels) < 0):
        # Public COTIC event-prediction datasets have no sequence-cluster labels.
        # A negative sentinel avoids fabricating purity while keeping JSON/logs
        # finite for long-running diagnostics.
