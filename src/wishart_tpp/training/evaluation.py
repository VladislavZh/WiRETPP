...
class ModelEvaluator:
    ...
    def active(
        ...
        *,
        ...
        for shard in self._shards(partition):
            cache = self.cache_builder.build(model, shard)
            # Independent repeats reduce Monte Carlo noise without changing the
            # number of matrices materialized by one decoder call.
            repeated_scores = [
                self.decoder.prior_predictive(
                    ...
                    population_df,
                    samples,
                    seed + repeat * 1_000_003 + offset * 10_007,
                    alpha,
                )
