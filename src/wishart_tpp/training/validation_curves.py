...
class ValidationCurveWriter:
    ...
    def record(
        ...
        *,
        ...
        horizons = np.asarray(
            [sequence.horizon for sequence in validation.sequences],
            dtype=np.float64,
        )
        if scores.shape != horizons.shape:
            raise ValueError("evaluation and validation partition are misaligned")
        bootstrap_seed = 8_000_021 + self.seed * 1_000_003 + int(cycle) * 10_007
        ...
        if not np.isclose(
            ...
            # ``Evaluation`` aggregates torch float32 scores whereas the
            # path artifact deliberately recomputes the ratio in float64.
            # A purely absolute tolerance is too strict for large-NLL real
            # datasets (notably StackOverflow) despite sub-ppm agreement.
            rtol=1e-6,
            atol=2e-6,
        ):
        ...
        # Write path values first.  A crash between the two replacements is
        # recoverable because recording the same point is an idempotent replace.
